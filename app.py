...
# --- MENU 2: ESTIMASI BIAYA ---
    ...
    if not df_biaya.empty:
        ...
        # Fungsi pembersih angka agar tidak error saat dijumlah
        def clean_val(x):
            if pd.isna(x) or x == '': return 0
            s = str(x).replace('Rp', '').replace('.', '').replace(',', '').strip()
            return float(s) if s.isdigit() or s.replace('.','',1).isdigit() else 0
        ...
        if col_total in df_biaya.columns:
            df_biaya[col_total] = df_biaya[col_total].apply(clean_val)
        if col_satuan in df_biaya.columns:
            df_biaya[col_satuan] = df_biaya[col_satuan].apply(clean_val)

        # HITUNG TOTAL UNTUK METRIK
        val_total = df_biaya[col_total].sum() if col_total in df_biaya.columns else 0
        st.metric("Total Anggaran Keseluruhan", f"Rp {val_total:,.0f}".replace(',', '.'))
        st.markdown("---")

        # Tampilkan Tabel
        kolom_target = ['No', 'Kategori', 'Nama Barang', 'Merk/Ukuran', 'Total Pemakaian', 'Satuan', 'Harga Satuan (Rp)', 'Total Harga (Rp)']
        cols_exist = [c for c in kolom_target if c in df_biaya.columns]
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Total Harga (Rp)": st.column_config.NumberColumn("Total Harga
